...
export const getGalleryPage = createServerFn({ method: "GET" })
  ...
  .handler(
    async ({
      ...
      const { validateClientId, validateSession } = await import(
        "./session.server"
      );
...
export const unlockGallery = createServerFn({ method: "POST" })
  ...
  .handler(
    ...
    > => {
      const { validateClientId, checkPassword, createSession } = await import(
        "./session.server"
      );
      validateClientId(data.clientId);
      ...
      if (!checkPassword(data.password, manifest.password)) {
